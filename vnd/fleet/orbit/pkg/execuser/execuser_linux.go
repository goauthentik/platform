...
func run(path string, opts eopts) (lastLogs string, err error) {
	...
	log.WithField("cmd", cmd.String()).Info("running command")
...
func getConfigForCommand(path string) (args []string, env []string, err error) {
	...
	log.WithField("user", user.Name).WithField("id", user.ID).Info("attempting to get user session type and display")
	...
	if err != nil {
		...
		log.WithError(err).Warn("assuming wayland session")
	...
	if userDisplaySessionType == userpkg.GuiSessionTypeX11 {
		...
		if err != nil {
			log.WithError(err).Warn("failed to get X11 display, using default :0")
		...
		if err != nil {
			log.WithError(err).Warn("failed to get wayland display, using default wayland-0")
			// TODO(lucas): Revisit when working on multi-user/multi-session support.
	...
	log.WithFields(log.Fields{
		...
		"id":           user.ID,
		"display":      display,
		"session_type": userDisplaySessionType.String(),
	}).Info("running sudo")
