...
func UserLoggedInViaGui() (*string, error) {
	...
	if err != nil {
		log.WithError(err).Debugf("failed to get user display session type for user %s", user.Name)
		return nil, nil
	}
	if displaySessionType == GuiSessionTypeTty {
		log.Debugf("user %s is logged in via TTY, not GUI", user.Name)
		return nil, nil
	}
...
func GetSELinuxUserContext(user *User) *string {
	...
	if _, err := exec.LookPath("runcon"); err != nil {
		log.Warn("runcon not available, returning nil for user context since we can't switch contexts")
		return nil
	}
	...
	if err != nil {
		log.Debugf("Error finding systemd process for user %s: %v", user.Name, err)
		return nil
	}
	pid := strings.TrimSpace(string(pidBytes))
	if pid == "" {
		log.Debugf("No systemd process found for user %s", user.Name)
		return nil
	}
	ctx, err := os.ReadFile("/proc/" + pid + "/attr/current")
	if err != nil {
		log.Debugf("Error reading SELinux context for user %s: %v", user.Name, err)
		return nil
	}
	...
	if context == "" {
		log.Debug("Empty SELinux context for user " + user.Name)
		return nil
	}
	return &context
}
