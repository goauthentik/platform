...
type FlowExecutor struct {
	...
	InteractiveChallenge SolverFunction
...
func (fe *FlowExecutor) getSolver(component StageComponent) (SolverFunction, error) {
	if fe.InteractiveChallenge != nil {
		return fe.InteractiveChallenge, nil
	}
	solver, ok := fe.solvers[component]
	if !ok {
		return nil, fmt.Errorf("unsupported challenge type %s", component)
	}
	return solver, nil
}
...
func (fe *FlowExecutor) solveFlowChallenge(challenge *api.ChallengeTypes, depth int) (bool, error) {
	...
	case string(StageRedirect):
		return true, nil
	default:
		solver, err := fe.getSolver(StageComponent(ch.GetComponent()))
		if err != nil {
			return false, err
		}
