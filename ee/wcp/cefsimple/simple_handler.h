...
public:
  ...
  bool OnResourceResponse(CefRefPtr<CefBrowser> browser,
    ...
    std::string str = "OnResourceResponse ProcessID: " +
    ...
    */
    ...
    if (strURL.length() >= strKey.length()) {
      if (strURL.substr(0, strKey.length()) == strKey) {
        ...
        Debug(strURL.c_str());
        std::string str = "OnBeforeResourceLoad ProcessID: " +
                          std::to_string(GetCurrentProcessId()) +
                          ", ThreadID: " + std::to_string(GetCurrentThreadId());
        Debug(str.c_str());
        ...
        std::string strState =
            strURL.substr(nPos, strURL.find("&", nPos) - nPos);
        Debug(std::string("State:: " + strState).c_str());

        if (strState == m_strState) //- else notify error?
        {
          browser->GetMainFrame()->LoadURL(
              CefString(m_pData->strBaseURL + g_strTokenEndpoint));
        } else {
          MessageBox(NULL, (LPCWSTR)L"Server response is forged.",
    ...
    if (strURL == (m_pData->strBaseURL + g_strTokenEndpoint)) {
      ...
      strPostData +=
          "&scope=openid%20email%20profile%20offline_access%20windows";
      CefRefPtr<CefPostData> pPostData = CefPostData::Create();
      CefRefPtr<CefPostDataElement> pPostDataElement =
      ...
      mapHeader.insert(std::make_pair<CefString, CefString>(
          "Content-Type", "application/x-www-form-urlencoded"));
      if (mapHeader.find("Accept") != mapHeader.end()) {
        mapHeader.erase("Accept");
      }
      mapHeader.insert(
          std::make_pair<CefString, CefString>("Accept", "application/json"));
  ...
  CefRefPtr<CefResponseFilter> GetResourceResponseFilter(
    ...
    if ((url == (m_pData->strBaseURL + g_strTokenEndpoint)) ||
        (url == (m_pData->strBaseURL + g_strJWKs))) {
      return this;
    }
  ...
  FilterStatus Filter(void *data_in, size_t data_in_size, size_t &data_in_read,
    ...
    std::string strDataOut =
        "Authenticating... please wait"; //- todo: displays twice at times (to
                                         // fix)
    memcpy(data_out, (void *)(strDataOut.c_str()), strDataOut.size());
    ...
    Debug(std::string("Data in: Size:" + std::to_string(data_in_size) +
                      " Read: " + std::to_string(data_in_read))
              .c_str());
    Debug(strIn.c_str());
    Debug(std::string("Data out: Size:" + std::to_string(data_out_size) +
                      " Written: " + std::to_string(data_out_written))
              .c_str());
    Debug(strOut.c_str());
    Debug("-------------------");
  ...
  void OnResourceLoadComplete(CefRefPtr<CefBrowser> browser,
    ...
    if (url == (m_pData->strBaseURL + g_strTokenEndpoint)) {
      ...
      m_strJWT = m_strResponseContent.substr(
          nPos, m_strResponseContent.find("\"", nPos) - nPos);
      m_strResponseContent =
          ""; // Delete JWT for security //- todo: do a proper overwrite
      Debug("JWT:");
      Debug(m_strJWT.c_str());
      browser->GetMainFrame()->LoadURL(
          CefString(m_pData->strBaseURL + g_strJWKs)); // Fetch JWKs
    }
    if (url == (m_pData->strBaseURL + g_strJWKs)) {
      Debug("JWKS:");
      Debug(m_strResponseContent.c_str());
      //- todo: Perform jwt-cpp verification
      // m_strJWT.at(m_strJWT.size() - 1) = 'R'; // induce error in JWT
      // verification
      auto decoded_jwt = jwt::decode(m_strJWT); // jwt

      Debug(std::string("JWT:: " + m_strJWT).c_str());
      Debug(
          std::string("JWT:: get_header " + decoded_jwt.get_header()).c_str());
      Debug(std::string("JWT:: get_payload " + decoded_jwt.get_payload())
                .c_str());
      ...
      if (ec.value() != 0) {
        MessageBox(NULL, (LPCWSTR)L"Authentication integrity check failed.",
                   (LPCWSTR)L"Error", MB_OK);
      } else {
        ...
        if (preferred_name != payload_json.end()) {
          ...
          Debug(
              std::string((*preferred_name).second.get<std::string>()).c_str());
          // {
          //   std::string strMsg = "Access verified for user: " +
          //   (*preferred_name).second.get<std::string>(); MessageBox(
          //       NULL,
          ...
          if (m_pData) {
            ...
            std::string strMsg = "Invalid data container pointer. Unable to "
                                 "login. You may re-try.";
            MessageBox(NULL, std::wstring(strMsg.begin(), strMsg.end()).c_str(),
                       (LPCWSTR)L"Internal Error", MB_OK);
          }
          CloseAllBrowsers(false);
        } else {
          MessageBox(NULL,
                     (LPCWSTR)L"Required information is missing from "
