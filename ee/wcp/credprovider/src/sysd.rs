...
/// Validates the token embedded in the sign-in redirect's URL, returning
/// the username on success. `None` covers both an unextractable token and
/// one `ak-sysd` rejects — either way the sign-in did not complete.
