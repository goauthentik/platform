  ...
  if (m_fIsLocalUser) {
    if (m_oHookData.IsCancel()) {
      ...
      *pcpsiOptionalStatusIcon = CPSI_WARNING;
      ...
      SPDLOG_DEBUG("GetSerialization->Protect");
      ...
      if (SUCCEEDED(hr)) {
        ...
        SPDLOG_DEBUG("GetSerialization->SplitDomainAndUsername");
        if (SUCCEEDED(hr)) {
          KERB_INTERACTIVE_UNLOCK_LOGON kiul;
          SPDLOG_DEBUG("GetSerialization->KerbInteractiveUnlockLogonInit");
          ...
          if (SUCCEEDED(hr)) {
            ...
            // necessary.
            SPDLOG_DEBUG("GetSerialization->KerbInteractiveUnlockLogonPack");
            hr = KerbInteractiveUnlockLogonPack(kiul, &pcpcs->rgbSerialization,
                                                &pcpcs->cbSerialization);
            if (SUCCEEDED(hr)) {
              ULONG ulAuthPackage;
              SPDLOG_DEBUG("GetSerialization->RetrieveNegotiateAuthPackage");
