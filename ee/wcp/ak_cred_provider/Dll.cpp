...
DllMain(__in HINSTANCE hinstDll, __in DWORD dwReason, __in LPVOID lpReserved) {
  ...
  switch (dwReason) {
    case DLL_PROCESS_ATTACH: {
      ...
      SPDLOG_TRACE("DllMain::DLL_PROCESS_ATTACH");

      DisableThreadLibraryCalls(hinstDll);
      SPDLOG_TRACE(std::string("DLL hInstance: " + std::to_string((size_t)hinstDll)).c_str());
      std::string strID = "DLL ProcessID: " + std::to_string(GetCurrentProcessId()) +
                          ", ThreadID: " + std::to_string(GetCurrentThreadId());
      SPDLOG_TRACE(strID.c_str());
    } break;
    case DLL_THREAD_ATTACH:
    case DLL_THREAD_DETACH:
      break;
    case DLL_PROCESS_DETACH:
      SPDLOG_TRACE("DllMain::DLL_PROCESS_DETACH");
      ak_teardown_sentry();
      ak_teardown_logs();
      break;
  }
