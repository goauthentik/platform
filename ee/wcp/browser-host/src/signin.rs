//! What this process reports to the credential provider, over the two
//! inherited standard handles: stdout carries exactly one [`HostReport`],
//! stdin the provider's request to close.
//!
//! Validating a `goauthentik.io://` redirect's token needs `ak-sysd`, which
//! this process cannot reach, so the URL is reported verbatim and
//! `credprovider` resolves it into an `AuthResult`.
...
impl Completion {
    ...
    /// successful sign-in; send-once is what stops the window-closed handler
    /// reporting a cancellation over the redirect.
...
///
/// Both were opened and access-checked in `credprovider` before this process
/// existed and handed over through `STARTUPINFOW`; this process's own token is
/// too restricted to open them itself.
pub struct StdPipes {
    pub result: File,
...
/// window is gone. Falls back to exiting outright, since a window that cannot
/// be found is still a process the provider is blocked on.
...
/// pipe and a read error both cancel, but only one of the three is the provider
/// asking — a bad handle reads as an immediate error and tears the window down
/// the instant it opens — so each logs separately.
        ...
        loop {
            match ak_ee_wcp_wire::read_host_command(&mut pipe) {
                Ok(Some(HostCommand::StartSignIn { url, header_token })) => {
                    ...
                    // Keep reading: until the redirect fires, this pipe is the
                    // only way the provider can call the flow off.
                }
...
mod tests {
    ...
    /// second send would overwrite the redirect URL with a cancellation: a
    /// sign-in that works and still lands the user back at the logon screen.
    #[test]
