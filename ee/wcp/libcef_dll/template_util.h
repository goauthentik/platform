...
template <typename T, typename = void> struct HasValidSize {
  bool operator()(const T *) { return true; }
};
template <typename T>
struct HasValidSize<T, typename std::enable_if_t<std::is_same<
                           decltype(T::size), std::size_t>::value>> {
  bool operator()(const T *s) { return s->size == sizeof(*s); }
};
template <typename T> struct HasValidSize<T, decltype(void(T::base.size))> {
  bool operator()(const T *s) { return s->base.size == sizeof(*s); }
};

template <typename T> inline bool has_valid_size(const T *s) {
