    ...
    : m_cRef(1)
{
    ...
    Credential::m_pProvCallShut = [this] () {
        this->ShutCefApp();
    };
...
void Provider::SetCefApp(sHookData* pData)
{
    ...
    if (m_pCefApp == nullptr)
    {
        ...
        std::string str = "SetCefApp ProcessID: " + std::to_string(GetCurrentProcessId()) + ", ThreadID: " + std::to_string(GetCurrentThreadId());
        Debug(str.c_str());
        ...
        exit_code = CefRunWinMainWithPreferredStackSize(wWinMain, hInstance,
                                                        lpCmdLine, SW_NORMAL); // nCmdShow: SW_NORMAL
        // if (exit_code >= 0) {
        ...
        Debug(std::string("app.get:::" + std::to_string((size_t)(m_pCefApp.get()))).c_str());
...
IFACEMETHODIMP Provider::QueryInterface(__in REFIID riid, __deref_out void** ppv)
{
    static const QITAB qit[] =
    {
        ...
        QITABENT(Provider, ICredentialProviderSetUserArray),    // IID_ICredentialProviderSetUserArray
        { 0 },
    };
...
IFACEMETHODIMP Provider::Advise(
...
IFACEMETHODIMP Provider::SetUsageScenario(
...
IFACEMETHODIMP Provider::GetCredentialCount(
    _Out_ DWORD* pdwCount,
...
IFACEMETHODIMP Provider::SetUserArray(_In_ ICredentialProviderUserArray* users)
{
            ...
            if (m_oCredentials.parrCredentials != nullptr)
            {
                ...
                for (DWORD dwIndex = 0; dwIndex < dwUserCount; ++dwIndex)
                {
                    ...
                    if (SUCCEEDED(hr))
                    {
                        ...
                        if (m_oCredentials.parrCredentials[dwIndex] != nullptr)
                        {
                            hr = m_oCredentials.parrCredentials[dwIndex]->Initialize(m_cpus, s_rgCredProvFieldDescriptors, s_rgFieldStatePairs, pCredUser);
                            if (SUCCEEDED(hr))
                            {
