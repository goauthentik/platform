    ...
    if (SUCCEEDED(hr))
    {
        ...
        if (m_hWndOwner)
        {
            if (hWndProcOrig == NULL)
            {
                ...
                // hWndProcOrig = SetWindowLongPtr(m_hWndOwner, GWLP_WNDPROC, (LONG_PTR)(std::function<LRESULT(HWND, UINT, WPARAM, LPARAM)>)[this] (
                //     _In_ HWND hWnd,
                //     _In_ UINT uMsg,
                //     _In_ WPARAM wParam,
                //     _In_ LPARAM lParam
                ...
                // });
                // hWndProcOrig = SetWindowLongPtr(m_hWndOwner, GWLP_WNDPROC, (LONG_PTR)WndProc);
            }
            if (hHook == NULL)
            {
                DWORD dwProcessID = 0;
    ...
    switch (uMsg)
    {
        case WM_NULL:
        {
            if (wParam == 100)
            {
                ...
                Debug(std::string("(m_oCefAppData.pCefApp) before: " + std::to_string((size_t)((m_oCefAppData.pCefApp).get()))).c_str());
                if (! (m_oCefAppData.pCefApp))
                {
                    if (m_pProvCallSet)
                    {
                        ...
                        // wait in custom message loop to process UI messages to avoid UI (cancel button) freeze
                    }
                    else
                    {
...
IFACEMETHODIMP Credential::Connect(IQueryContinueWithStatus* pqcws)
{
    ...
    if (m_pCredProvCredentialEvents)
    {
        ...
        if (hr != S_OK) {
            ...
            MessageBox(
                NULL,
                std::wstring(L"Error retrieveing config.").c_str(),
