...
Credential::Initialize(CREDENTIAL_PROVIDER_USAGE_SCENARIO cpus,
  ...
  if (SUCCEEDED(hr)) {
    hr = SHStrDupW(L"Authentik Credential Provider",
                   &m_rgFieldStrings[FI_LABEL]);
  }
  if (SUCCEEDED(hr)) {
    hr = SHStrDupW(L"Authentik Credential Provider",
                   &m_rgFieldStrings[FI_LARGE_TEXT]);
  }
  if (SUCCEEDED(hr)) {
...
Credential::Advise(_In_ ICredentialProviderCredentialEvents *pcpce) {
  ...
  if (SUCCEEDED(hr)) {
    ...
    std::string str =
        "Advise:: hwndOwner: " + std::to_string((uint64_t)hwndOwner) + " ";
    ...
    if (hwndOwner) {
      if (hWndProcOrig == NULL) {
        ...
        hWndProcOrig = SetWindowLongPtr(hwndOwner, GWLP_WNDPROC,
...
IFACEMETHODIMP Credential::UnAdvise() {
  ...
  //     m_pCredProvCredentialEvents->OnCreatingWindow(&hwndOwner);
  // }
  // std::string str = "UnAdvise:: hwndOwner: " +
  // std::to_string((uint64_t)hwndOwner); Debug(str.c_str()); if (hwndOwner)
  // {
  //     DWORD dwProcessID = 0;
  //     DWORD dwThreadID = GetWindowThreadProcessId(hwndOwner, &dwProcessID);
  //     std::string strHookInfo = "UnAdvise Hook:: Process ID: " +
  //     std::to_string(dwProcessID) + ", Thread ID: " +
  //     std::to_string(dwThreadID); Debug(strHookInfo.c_str());
  // }
  if (hHook) {
...
IFACEMETHODIMP Credential::Connect(IQueryContinueWithStatus *pqcws) {
  ...
  if (m_pCredProvCredentialEvents) {
    ...
    if (hr != S_OK) {
      ...
      MessageBox(NULL, std::wstring(L"Error reading GUID.").c_str(),
    ...
    { // Read URL from registry
      ...
      while ((strURL.find_last_of('\0') == (strURL.size() - 1)) &&
             (strURL.size() > 0)) {
        strURL = strURL.substr(0, strURL.size() - 1);
      }
      // Remove trailing `/`(s)
      while ((strURL.find_last_of("/") == (strURL.size() - 1)) &&
             (strURL.size() > 0)) {
        strURL = strURL.substr(0, strURL.size() - 1);
      }
    ...
    { // Read Client ID from registry
      ...
      // Remove trailing null
      while ((strClientID.find_last_of('\0') == (strClientID.size() - 1)) &&
             (strClientID.size() > 0)) {
    ...
    // Authentik sign-in page does not load)");
        ...
        if (SUCCEEDED(hr)) {
          ...
          if (SUCCEEDED(hr)) {
            ...
            if (SUCCEEDED(hr)) {
              ...
              // that we have all the information we need and it should attempt
              // to submit the serialized credential.
