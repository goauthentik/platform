...
struct sHookData {
  ...
  void UpdateUser(const std::string &strUser) { strUsername = strUser; }
  void UpdateBaseURL(const std::string &strURL) { strBaseURL = strURL; }
...
public:
  ...
  static LRESULT CALLBACK Credential::CallWndProc(_In_ int nCode,
                                                  _In_ WPARAM wParam,
                                                  _In_ LPARAM lParam);
  static LRESULT CALLBACK Credential::WndProc(_In_ HWND hWnd, _In_ UINT uMsg,
                                              _In_ WPARAM wParam,
                                              _In_ LPARAM lParam);
  static HHOOK hHook;
...
private:
  ...
  m_cpus; // The usage scenario for which we were enumerated.
  CREDENTIAL_PROVIDER_FIELD_DESCRIPTOR
  m_rgCredProvFieldDescriptors[FI_NUM_FIELDS]; // An array holding the type
                                               // and name of each field in
                                               // the tile.
  FIELD_STATE_PAIR
  m_rgFieldStatePairs[FI_NUM_FIELDS];    // An array holding the state of each
                                         // field in the tile.
  PWSTR m_rgFieldStrings[FI_NUM_FIELDS]; // An array holding the string value of
                                         // each field. This is different from
                                         // the name of the field held in
                                         // _rgCredProvFieldDescriptors.
  PWSTR m_pszUserSid = nullptr;          //-
  PWSTR m_pszQualifiedUserName =
      nullptr; // The user name that's used to pack the authentication buffer
  ...
  bool m_fShowControls =
      false; // Tracks the state of our show/hide controls link.
  bool m_fIsLocalUser =
      false; // If the cred prov is assosiating with a local user tile
