...
public:
  ...
  // ICredentialProvider
  IFACEMETHOD(Advise)(_In_ ICredentialProviderEvents *pcpe,
                      _In_ UINT_PTR upAdviseContext);
  IFACEMETHOD(UnAdvise)(void);

  IFACEMETHOD(SetUsageScenario)(CREDENTIAL_PROVIDER_USAGE_SCENARIO cpus,
                                DWORD dwFlags);
  IFACEMETHOD(SetSerialization)(
      _In_ const CREDENTIAL_PROVIDER_CREDENTIAL_SERIALIZATION *pcpcs);

  IFACEMETHOD(GetFieldDescriptorCount)(_Out_ DWORD *pdwCount);
  IFACEMETHOD(GetFieldDescriptorAt)(
      DWORD dwIndex,
  ...
  IFACEMETHOD(GetCredentialAt)(
      DWORD dwIndex, _COM_Outptr_ ICredentialProviderCredential **ppcpc);
