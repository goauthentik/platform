...
use crate::{keyring::{self, KeyringError}, prelude::BoxError};
pub trait CacheData {
    ...
    T: CacheData + Clone + Serialize + DeserializeOwned,
{
    ...
    pub async fn set(self, val: T) -> Result<(), CacheError> {
        log::debug!("Writing to cache");
        let serialized = serde_json::to_string(&val).map_err(|e| CacheError::Other(Box::new(e)))?;
        keyring::set(
        ...
        .await
        .map_err(|e| CacheError::Other(Box::from(e)))
    }
