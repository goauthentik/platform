...
pub mod session_manager_server {
    #![allow(
        unused_variables,
        dead_code,
        missing_docs,
    ...
    use tonic::codegen::*;
    /// Generated trait containing gRPC methods that should be implemented for use with SessionManagerServer.
    #[async_trait]
    pub trait SessionManager: std::marker::Send + std::marker::Sync + 'static {
    ...
    #[derive(Debug)]
    pub struct SessionManagerServer<T> {
        inner: Arc<T>,
    ...
    impl<T, B> tonic::codegen::Service<http::Request<B>> for SessionManagerServer<T>
    where
        T: SessionManager,
        ...
        fn call(&mut self, req: http::Request<B>) -> Self::Future {
            match req.uri().path() {
                "/session.SessionManager/SessionStatus" => {
                    ...
                    struct SessionStatusSvc<T: SessionManager>(pub Arc<T>);
                    impl<
                        T: SessionManager,
                    ...
                    for SessionStatusSvc<T> {
                        ...
                        fn call(
                            ...
                            let fut = async move {
                                <T as SessionManager>::session_status(&inner, request).await
                            };
                            Box::pin(fut)
                        }
                    }
                ...
                "/session.SessionManager/CloseSession" => {
                    #[allow(non_camel_case_types)]
                    struct CloseSessionSvc<T: SessionManager>(pub Arc<T>);
                    impl<
                        T: SessionManager,
                    ...
                    for CloseSessionSvc<T> {
                        ...
                        fn call(
                            ...
                            let inner = Arc::clone(&self.0);
                            let fut = async move {
                                <T as SessionManager>::close_session(&inner, request).await
                            };
                            Box::pin(fut)
                        }
                    }
    ...
    impl<T> Clone for SessionManagerServer<T> {
        fn clone(&self) -> Self {
            let inner = self.inner.clone();
            Self {
                inner,
    ...
    pub const SERVICE_NAME: &str = "session.SessionManager";
    impl<T> tonic::server::NamedService for SessionManagerServer<T> {
