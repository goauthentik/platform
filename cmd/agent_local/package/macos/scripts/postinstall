...
chromiumSystemWidePath="/Library/Application Support/Chromium/NativeMessagingHosts"
edgeSystemWidePath="/Library/Microsoft/Edge/NativeMessagingHosts"
firefoxSystemWidePath="/Library/Application Support/Mozilla/NativeMessagingHosts"
...
if [ ! -d "$chromiumSystemWidePath" ]; then
    echo "$chromiumSystemWidePath doesn't exist, creating directory"
    mkdir -p "$chromiumSystemWidePath"
fi
...
if [ ! -d "$firefoxSystemWidePath" ]; then
    echo "$firefoxSystemWidePath doesn't exist, creating directory"
    mkdir -p "$firefoxSystemWidePath"
fi

# Copy the manifest file to the system-wide directory
cp -v "/Applications/authentik Agent.app/Contents/Resources/browser-host-chrome.json" "$chromeSystemWidePath/io.goauthentik.platform.json"
cp -v "/Applications/authentik Agent.app/Contents/Resources/browser-host-chrome.json" "$chromiumSystemWidePath/io.goauthentik.platform.json"
cp -v "/Applications/authentik Agent.app/Contents/Resources/browser-host-chrome.json" "$edgeSystemWidePath/io.goauthentik.platform.json"
cp -v "/Applications/authentik Agent.app/Contents/Resources/browser-host-firefox.json" "$firefoxSystemWidePath/io.goauthentik.platform.json"
