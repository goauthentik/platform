            ...
            tokio::runtime::Builder::new_current_thread()
                ...
                .and_then(|rt| {
                    rt.block_on(async move {
                        let context = LAContext::new().wrap_err("failed to create LocalAuthentication context")?;
                        ...
                        match context.can_evaluate_policy(policy) {
                            ...
                            Err(e) => return Err(Error::from(e).wrap_err("LocalAuthentication: policy not evaluable")),
                        }
                        match context.evaluate_policy_async(policy, &msg).wrap_err("failed to initiate policy evaluation")?.await {
                            Ok(b) => Ok(b),
                            Err(LAError::AppCancel(_) | LAError::BridgeFailed(_)) => Ok(false),
                            Err(e) => Err(Error::from(e).wrap_err("LocalAuthentication: policy evaluation failed")),
                        }
                    })
                });
